...
interface IBoardItemPage {
  el?: any;
  data?: any;
  // onClickDataSaveLocal: (event: MouseEvent<HTMLButtonElement>) => void;
}
...
export default function BoardItemPage(props: IBoardItemPage) {
  ...
  const onClickDeleteBoard = (el) => () => {
    const basket = JSON.parse(localStorage.getItem("basket") || "[]");
    const deleteBasket = basket.filter((basketEl) => basketEl._id !== el._id);
